...
// 下面 rgb 是按常见六色墨水屏的「经验近似值」（真机六色偏暗、不饱和，并非纯三原色），已比纯色更接近真机。
// ⚠️ 要最准：传一张彩条图/纯色块到设备，拍屏，用取色器量出每种颜色的实际 RGB，替换对应 rgb（nibble 别动）。
const PALETTE = [
  { nibble: 0x0, rgb: [0, 0, 0], name: '黑' }, // 黑、白保留纯值当无彩色锚点，避免灰阶被错归到彩色
  { nibble: 0x1, rgb: [255, 255, 255], name: '白' },
  { nibble: 0x2, rgb: [235, 215, 35], name: '黄' }, // 偏土黄
  { nibble: 0x3, rgb: [180, 45, 40], name: '红' }, // 偏砖红
  { nibble: 0x5, rgb: [45, 65, 145], name: '蓝' }, // 偏藏蓝
  { nibble: 0x6, rgb: [55, 120, 70], name: '绿' } // 偏暗绿
]
