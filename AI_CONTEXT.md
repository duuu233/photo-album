...
- 职责：
  ...
  - **不再调用任何「我的图库」接口**（2026-08-17）：再次投屏与删除都只吃这份投屏成功记录，
    页面数据只有 `getDevices`（下拉分类）与 `getProjectionRecords`（网格）两个来源；
  - 使用记录自己的 `imgIndex` 定位设备物理槽位；
  - 单选/多选再次投屏（多选走批量传输链路）、删除设备槽位 + 来源投屏记录；
  ...
  - 默认选中的设备（`list.js pickDefaultFilter`，2026-08-17 收成三档）：
    ...
    原「第一台有照片的设备」随图库列表一起删除：判据取自图库照片，而本页数据源是投屏记录）。
- 关键文件：
...
```text
投屏成功记录（getProjectionRecords + deviceUploadState=1，按 userProductId 分类）
  ← 删除与再次投屏**都只吃这一份数据**，不再关联云端图库（2026-08-17）
  → 删除：只取这条投屏记录自己的 imgIndex
          → 确认设备完整身份和活动连接
          → 读 0x01 的 IMG_MASK 分拨：设备上已无的槽位跳过，仍在的组成 12 字节 IMG_INDEX_MASK 发 0x12
          → 无论设备成功 / 跳过 / 回 0x05·0x07 / 真失败，都继续删来源投屏记录（端上到此为止）
  → 再次投屏：直接用记录自己的 img → 连接该记录所属设备
          → 写 pendingProjection → 走正常预览/投屏链路建**新记录**（多选即批量传输）
          → 结果页：seekink 抖动接口出帧 → 读 0x01 取 firstFreeIndex → BLE 发帧 → 记账 imgIndex
```

2026-08-17 起端上不再调 `getUserProductImgList` / `delUserProductImg`（连同 `api.getAlbumPhotos` /
`deleteAlbumPhotos` 一起删除）。原来图库列表只喂两处：再次投屏取「原图」、默认设备规则「第一台有
照片的」。前者的理由（记录 `img` 是设备比例图）自 2026-07-22 `setUserProductUpload` 改传原图起
已失效——记录图与图库图本就是同一次上传的同一张；后者判据与本页数据源不同源，一并去掉。
图库照片的清理归后端。

删除链路不按设备掩码/上传顺序/列表顺序**推算**缺失索引：任一选中项缺少有效 `imgIndex` 时整批
终止，不删除设备或后端记录。

`0x01` 的 `IMG_MASK` 只读来回答「这个槽位现在还有没有图」（2026-08-10 下午加）：多手机共用一台
设备时，另一端删过的那张在本机列表里还在，把它的空槽位一起下发会让固件回 `0x07`、**整批**被打回。
现在空槽位跳过、仍在的照删，`0x05`/`0x07` 也按「设备侧本来就没有」放行，后端记录一律删掉。
掩码读不到时不做过滤，绝不把整批判成「已不在设备上」。

**设备侧与记录侧互不阻断（2026-08-20）**：顺序仍是「先发 0x12 → 再调投屏记录删除接口」，但 Flash
写失败(`0x04`)、传输中断(`0x09`)、断连超时等真失败**不再中止整批**，记录照删；记录接口失败也不回滚
设备。两半的结果合并成一条提示。代价：设备真没删掉时相框上会留一张「相册里看不到、端上也没有它
`imgIndex`」的幽灵图，只能靠一键清空清理（产品已确认接受）。

**唯一例外：设备繁忙 `0x0B`（2026-08-24，协议文档口径）**——只提示「当前电子纸设备繁忙，请稍后重试」
并**整批中止，任何数据都不删**（设备侧没删、后端记录不删、列表不刷新、选中态保留供直接重试）。
繁忙意味着指令被设备**主动拒绝、根本没执行**（规格书 v1.5 §6.6.1，刷屏/写 Flash 的忙窗口约几秒），
重试就能成功，不该像那几类「可能真删了一半」的失败一样先把记录删掉留幽灵图。
判据是 `protocol.isBusyError()`（先认 `resultCode=0x0B`，再兜文案），`0x01` 掩码回读与 `0x12` 应答
两处都拦；用例见 `tests/album-delete-independent.test.js` 第 ⑥/⑥'/⑦ 组。
...
8. AI 图片压缩与投屏图片缩放目标不同：
   ...
   - 投屏缩到设备物理分辨率以保画质和帧布局。
9. BLE/OTA 错误应保留具体来源：
   - 我方后端接口错误通常带“接口-”（request.js 统一加）；
   - AI（BoltStar，独立域名）错误带“接口(AI)-”（`utils/ai-i18n.js`）；
   - 第三方接口错误带“接口(第三方)-”（目前仅 seekink 出帧，见 `utils/dithering.js`）；
   - seekink 出帧的失败对用户统一显示“接口(第三方)-网络超时，图片转化失败，请重新尝试”，
     微信原文只进日志与 `error.raw`，不要再把英文 errMsg 丢到失败页；
   - 设备/BLE 错误通常带“设备-”；
   - 新增来源前缀时，`toast.js` / `device-ble.js` / `ota-ble.js` / `ai-i18n.js` / `device/detail`
     里“已带前缀就不再叠加”的判断要一起放行，否则会被再叠一层“小程序-”；
