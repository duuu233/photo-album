...
## 操作日志（最新在上）
- **2026-07-27**：设备电量改为**每次展示都实时发送 BLE `0x04 GET_BATTERY`**。取消
  `utils/device-info.js` 的 15 秒结果复用，首页、设备列表、设备详情、连接成功页和 OTA 页不再读取
  `selectedDevice`/Storage 中的历史电量；进入展示态先置 `--`，收到本次 `0x04` 后再显示，失败仍为 `--`。
  同一设备只共享正在进行中的请求以避免 `CMD_PENDING`，完成后不缓存。控制台统一打印
  `[设备电量][0x04]` 的 `rawBattery`，便于判断后续波动是否来自设备应答。关联：
  `utils/battery.js`、`utils/device-info.js`、`utils/active-device.js`、`app.js`、
...
- **2026-07-27**：修复**两台同尺寸设备点击 A 却连接到 B**。根因是扫描广播仅含 4 字节
  ...
  同步采用「后端记录主键/完整 ID 优先」，设备自定义名称不参与物理身份判断。关联：
...
| 我的设备 | 设备连接流程 | ➖ | ✅ | - | 广播短 ID + 尺寸仅用于筛选候选；连接后读取 `0x01` 完整设备 ID 严格确认，校验通过后才认领会话并写直连缓存。身份不一致时断开、排除错误候选并重扫，同时清理被污染的缓存。设备自定义名称不作为物理身份依据（2026-07-27） |
...
| 我的设备 | 设备电量 | ➖ | ✅ | - | 每次进入需要展示电量的业务页面，均实时发送 BLE `0x04 GET_BATTERY`；不使用 15 秒结果缓存或跨页/落盘历史电量。读取失败显示 `--`，控制台记录设备原始返回值（2026-07-27） |
