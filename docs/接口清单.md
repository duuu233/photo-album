...
| 个人信息 | 绑定/修改邮箱 | `POST /Client/User/changeUserEmail` | ✅ | `changeUserEmail` | 小程序绑定/修改邮箱共用；body `userEmail`+`verifyCode`+`password`+`confirmPassword`（密码 md5 32位小写）；`device/language/terminal/userToken` 经 header；绑定后邮箱+密码可用于 App 登录 |
