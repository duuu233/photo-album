...
| AI 助手 | 能否发起 AI 对话 | `GET /Client/Order/chkAiDialogue` | ✅ | `checkAiDialogue` | 2026-08-12 新增。无入参（用户由公共参数 `userToken` 带出），`retData` **直接是布尔**：true=可发起。端上封装成 `aiToken.canDialogue()`，聊天页每次发送前调一次；⚠️ **接口本身失败一律放行**（读不到判据就锁死 AI 是更糟的失败模式，`/chat` 侧服务端会再拒一次）。它取代了端上按余额比大小的老闸（`LIMIT_ENABLED`/`hasBalance` 已删除） |
