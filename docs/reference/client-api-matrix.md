...
星币管理套餐卡：原约单价文案改为带删除线的静态 `￥0.01`，仅为展示占位，后续按正式接口字段接入；实际购买金额继续使用套餐售价。卡片支持至少六位整数及两位小数，保留横滑和选中样式。确认购买页的已有单价计算不受本次调整影响。
...
| Token管理 | 商品（套餐）列表 | `GET /Client/Order/getGoodsList` | ✅ | `getPackages` | 出参 `goodsId`/`num`/`giveNum`/`amount`/`currencySymbol`/`wxProductId`/`appleProductId`，另有划线原价 `marketAmount`/`marketAmountEnglish`/`marketAmountFan`/`marketAmountJapanese`。⚠️ **原价只展示、不参与下单与支付金额**：端上按当前语种码（0/1 英文、2 简中、3 繁中、4 日文）取对应字段，语种专属字段不存在时回落 `marketAmount`（兼容服务端已按语种投影的响应），存在但为空则**不跨语种回退**（避免把别的币种金额贴到当前语种）；`0` 有效，缺失/空/负数/非有限数一律不展示、卡片留空行。`currencySymbol` 与金额必须同源，后端没给才回落 `¥`。`wxProductId` 即微信侧道具 id，**由服务端签进 signData**，端上不直接使用。`unitPrice`(integer) 端上不用，单价按「含赠送总数」自算 |
