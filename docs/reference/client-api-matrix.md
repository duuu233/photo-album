...
| 我的图库 | 列表 | `GET /Client/UserProduct/getUserProductImgList` | ✅ | `getUserProductImgList` | 出参必须含 `imgIndex`(String，设备槽位 bit 索引，0 合法)：删除照片只认该值；缺失/非法时整批终止，不从设备掩码或列表顺序推算。2026-08-04 起该列表不再直接渲染，只作为「我的相册」的槽位账本与原图来源 |
