...
- [2026-08-07 SSE 接入文档核对（「-改」版）：进度补间 + 一键生图即时上屏](changes/2026-08-07-SSE文档核对与进度补间.md) — 仅小程序；协议本身没变（解析层一行未动），新增可选 `model_type`；后端只发里程碑改由前端每 80ms 补间出中间读数（demo 图上的 47%），进度文案改成认 `stage`（拆出 85「正在下载图片…」）；一键生图改成「气泡先上屏、再建会话」，消掉点下去干等约 1s 的空窗；纠正「`image` 约 50% 到」的旧理解（实际排在 `progress 90` 之后）
