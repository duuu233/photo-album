...
- [2026-08-11 OTA：删除整套测试升级模块 + 修好「点了没反应」的真实升级](changes/2026-08-11-OTA删除测试模块与真实升级修复.md) — 仅小程序，Flutter 待核对；删掉「OTA测试升级」行、「固件升级」长按测试、`test=1`/`dry=1`、`dryRunFirmware`/`mockFirmwareBuffer`（干跑让「升级成功」有了两种含义）。真实升级的「点了没反应」根因是**按钮画得出来、`canUpgrade` 却因未连接恒 false**：现改为按钮「连接并升级」+ 升级前就地自动扫连，所有分支都不再静默 return
