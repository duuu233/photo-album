...
- [2026-08-10 「我的相册」删除跳过设备上已经没有的槽位](changes/2026-08-10-相册删除跳过设备已无的槽位.md) — 仅小程序，Flutter 待同步；多手机共用一台设备时，另一端删过的那张在本机列表里还在，空槽位一起下发 `0x12` 会被固件回 `0x07` 打回**整批**（提示「掩码不一致(该位置已有图/索引越界)」）。改为先读 `IMG_MASK` 分拨、空槽位跳过、`0x05`/`0x07` 放行，后端记录一律删掉；顺带修好 08-10 上午起就跑不通的 `tests/album-slot-index.test.js`
- [2026-08-10 AI 助手八项优化（顶栏标题居中、会话入口、Token 位让出等）](changes/2026-08-10-AI助手八项优化.md) — 仅小程序；聊天页不再渲染 `custom-tabbar`，顶栏改标题居中 + 返回/会话两颗按钮，Token 展示位从右上角让出
- [2026-08-08 微信虚拟支付对接（Token 由 mock 切真实后端）](changes/2026-08-08-微信虚拟支付对接.md) — 仅小程序；接入 `/Client/Order/*` 与 `/Client/Pay/getWxVirtualPayQueryOrder`，新增 `utils/wx-virtual-pay.js`；到账判据是**服务端余额变多**（退避轮询 ~9.4s）而非支付 success 回调。⚠️ `addOrder` 出参仍缺 `signData`/`paySig`/`signature`
