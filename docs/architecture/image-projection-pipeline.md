...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| 请求 MTU | 500 | 建连**协商两次**（间隔 120ms）并取小：部分安卓栈在服务/特征刚发现完时接受 `setBLEMTU` 并回调成功，但 ATT MTU 交换还没落定，回调与 `getBLEMTU` 都回显虚高的请求值——按它分包会被链路静默丢弃、传十几包后彻底卡死。`setBLEMTU` 直接报错的栈按 247 再请求一次 |
