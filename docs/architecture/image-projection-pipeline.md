...
> **用户侧看到的只有一句**（2026-09-03 二改起，两端同文案）：
> `图片文件错误，请尝试通过工具转码后重新投屏`（App 走 l10n `castFailureImageFileBroken`，中/英/日）。
> 文案沿革：① 「后端返回的不是电子纸设备要的六色4bpp帧：收到 X 字节…」——**帧自 2026-07-22（App 07-23）
> 起就不是我方后端出的**（seekink 抖动接口生成，`setUserProductUpload` 返回的 `.bin` 早已不下载使用），
> 那句话会把用户和客服全引到我方接口上去；② 「图片转化出错，请重新投屏」——不再甩锅后端，但**重投同一张
> 只会原样再失败一次**（帧字节数只跟上传图像素走）；③ 现文案——根因在图片文件本身过不了端上出图那一步，
> 换图或先转码是唯一能自救的动作。排查用的数一个没少，只进日志（清单见下）。
> ⚠️ 小程序 `classifyFailureMessage` 的 `connLost` 第一项「抛错这一刻未连接」仍会把它盖成
> 「电子纸设备未连接…」，属既有行为。
...
常见根因：预览页出图失败**静默回退原图**。`compressForUpload` 只把长边等比缩到设备长边、
**不改比例也不补边**，补不出设备的另一条边，于是上传图带着原图比例进了抖动接口。
修的是预览导出那一环（`preview.js` 恒导出设备物理分辨率），不是文案，也不是第三方。

⚠️ **出图有四个静默逃生口**（`preview.js`，Flutter 同构于 `_startCast` 的 `out ?? src`）：
`coverCropOne` 的 `fail:` / `.catch()` / 「尺寸不可用」三处 + `bakeEditedImage` 的 `.catch()`，
**除最后一个外原本都不打日志**——所以「设计上每张都导出设备物理分辨率」与「运行时这张悄悄没导出」
的落差长期看不见。2026-09-03 起补了排查日志（两端）：

| 日志 | 位置 | 看什么 |
| --- | --- | --- |
| `[预览][出图]` | `prepareProjectionImages` / `_startCast` | 本张走哪条分支 + 产物是否等于设备物理分辨率（不符即 warn） |
| `[预览][coverCrop]` | `coverCropOne` / coverCropToSize 分支 | 源图像素、`type`、`orientation`；出图成功/回退原图 |
| `[预览][导出]` | `_exportCanvas`（仅小程序） | canvas 三个失败出口分开：画布不可用 / `createImage` 解码失败 / `canvasToTempFilePath` 失败 |
| `[投屏][出帧]` | `fetchDitheringFrame` / `_fetchDitheringFrame` | 喂给抖动接口的那张图（小程序含**像素尺寸**，App 只有字节数） |
| `[AI][图片格式]` | `chat.js downloadAiImage`（仅小程序） | 读**文件头魔数**认真实编码——扩展名跟着 URL 走，不可信 |

完整定位过程见 [2026-09-03 AI 图投屏帧字节数对不上定位](../changes/2026-09-03-AI图投屏帧字节数对不上定位.md)。
