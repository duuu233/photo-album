...
| 广播 Device ID | 厂商广播包中的设备地址：老固件 4 字节；**2026-08-04 起新固件为完整 6 字节 MAC** | **否**，只用于缩小扫描候选范围与列表展示。⚠️ 即使新固件长度已够，也**不得**用它认领会话/判重/入库——广播是明文、可被伪造重放，稳定身份一律以连上后 `0x01` 读回的值为准。2026-08-11 起这条由 `session.verifiedSerials` 显式保证（此前靠"广播只有 4 字节、过不了 `isComplete`"隐式挡住，扩到 6 字节后该隐式前提失效） |
...
后端设备记录（完整 6 字节 ID）
  → 查找已连接会话
  → 若完整 ID 匹配（只认 0x01 核实过的 verifiedSerials），直接复用
  → 否则扫描广播，只筛选候选
      ├─ 强匹配：整串相等 / 短的是长的前后缀 / 整串字节序相反
      ├─ 弱匹配：4 字节锚段重合（强匹配落空时才用）
      └─ 都落空：同型号 + 确认是本产品广播的最强信号那台（兜底候选，仅在扫描窗口走满后）
  → 连接候选 BLE deviceId
  → 发送 0x01 GET_INFO
...
候选筛选一路放宽、身份校验一步不让，是有意的分工（2026-08-11）：广播 Device_ID 的长度与写法由固件
说了算，OTA 一改（4→6 字节、换段、换字节序）端上就可能一个候选都筛不出，用户看到「设备就在眼前却搜不到」。
放宽候选的代价只是最坏多连错一次（连上后被 `0x01` 打回、排除、重扫），收益是**任何广播写法下都还能连上**。
详见 [2026-08-11 变更记录](../changes/2026-08-11-OTA后设备ID四六字节兼容与全链路日志.md)。
...
- 身份登记表只收完整 6 字节 ID，且只按记录自己的主键登记；广播 ID（无论 4 还是 6 字节）、BLE 句柄
  不得入表，也不得借用另一条记录的主键登记，否则等于用缓存制造串台。
- 会话认领与身份回写只取 `session.verifiedSerials`（`0x01 GET_INFO` 读回的那份）。
  `session.serials` 里混着扫描广播登记的值，广播扩到 6 字节后"长度够"已不能证明它可信。
