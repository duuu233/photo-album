...
- 发送前的星币闸（2026-08-12）：`utils/ai-token.canDialogue()` → `GET /Client/Order/chkAiDialogue`。
  ...
  **不能发＝`retCode 403`**＋`retMsg"token余额不足，需要最低余额：30.0 token"`＋`retData null`
  ...
  拦下时弹「星币不足」（右键「去充值」**直达套餐确认**：带 `buy=1` 进星币管理页，
  那一页拉完套餐自己把确认页推上来；2026-09-18 起弹窗里还显示一条
  **当前余额**，绑 `tokenBalanceText`，`guardAiDialogue` 那次刷新落地时自己更新），
  `/chat` 一个请求都不发、用户气泡一条都不上屏、
  **草稿原样还回输入框**（余额见底的用户每次发送都会撞上这一步）。
  ⚠️ 余额刷新**读不到时保持上一次的数字**（`refreshTokenBalance`）：接口抖一下就把它抹成
  `--`，等于把一次网络故障写成「你一分不剩」。
