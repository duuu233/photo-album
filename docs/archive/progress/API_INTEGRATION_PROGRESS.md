...
> 状态：archived  
> 当前接口状态：[客户端接口矩阵](../../reference/client-api-matrix.md)  
> 说明：本文保存接入过程和历史落点，不再作为当前接口状态的权威来源。
...
> 2026-06-11 增量：对照 Swagger 最新接口，补齐用户接口、设备接口及基础新增接口（`getBasicData`、`setFileUpload`），并新增了 Swagger 新出现的投屏记录接口（`getUserProductImgRecordList`、`delUserProductImgRecord`）。当前接口清单见 [客户端接口矩阵](../../reference/client-api-matrix.md)。
>
