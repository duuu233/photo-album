...
> 状态：archived-design  
> 当前设计：[图片投屏流水线](../../architecture/image-projection-pipeline.md)  
> 说明：正文记录服务器生成 frame.bin 的目标方案；当前小程序主链路已改为 seekink 出帧。
...
- **2026-07-24（二）**：**seekink 抖动接口域名升 https**。由 `http://cloud.seekink.cn:8091` 迁到
  ...
  本文件及 `docs/reference/client-api-matrix.md`、`docs/changes/2026-07-22-照片预览需求调整.md` URL 引用。
...
- **2026-07-23（二）**：**再次/重新投屏并入正常链路，imgBle(.bin) 直传整链路删除**（后端已不再
  ...
  方法保留）、`docs/reference/client-api-matrix.md`。**Flutter 侧 07-23 只改了服务层、页面层残留未清（当时无 SDK 未编译，
...
- **2026-07-23**：**抖动接口鉴权由写死联调 token 改为接口动态获取**：新增
  ...
  `subpackages/projection/preview/preview.js`、`docs/reference/client-api-matrix.md`。（Flutter 已于 07-23/07-24 同步，
...
- **2026-07-22**：**正常投屏的设备帧改由第三方 seekink 抖动接口生成，不再下载后端转码的 .bin**
  ...
  - 关联：`subpackages/projection/result/result.js`、`utils/dithering.js`、`subpackages/projection/preview/preview.{js,wxml,wxss}`、
    `docs/reference/client-api-matrix.md`、`docs/changes/2026-07-22-照片预览需求调整.md`。
