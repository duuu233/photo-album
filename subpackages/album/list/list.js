...
//     最后把两边结果合并成一条提示。
//     **唯一例外：设备繁忙(0x0B)**（2026-08-24，接口/协议文档口径）——只提示「设备繁忙」并
//     整批中止，**任何数据都不删**（设备侧没删、后端记录也不删、列表不刷新、选中态保留）。
//     繁忙代表指令被设备**主动拒绝、根本没执行**（规格书 v1.5 §6.6.1），稍后重试即可成功，
//     不该像其它真失败那样先把记录删掉留一张幽灵图。
//
...
Page(fold.adapt({
  ...
  async confirmDeleteSelected() {
    ...
    try {
      ...
      // 设备繁忙(0x0B)：连 0x01 都被主动拒了，设备正忙着别的指令，后面的 0x12 也必然被拒。
      // 按 2026-08-24 口径**整批中止、什么都不删**，只提示繁忙（见第 4 步注释）。
      if (protocol.isBusyError(error)) {
        wx.hideLoading()
        console.warn(
          '[相册删除] 回读设备掩码(0x01)时设备繁忙，整批中止，不删除任何数据：',
          (error && error.message) || error
        )
        toast.warn({ title: protocol.BUSY_MESSAGE, icon: 'none' })
        return
      }
      console.warn(
    ...
    //
    // ⚠️ 2026-08-24 例外（按接口/协议文档口径）：**设备繁忙(0x0B) 恢复为整批中止**——
    // 提示「当前电子纸设备繁忙，请稍后重试」，设备侧、后端记录一律不动，列表不刷新、选中态保留，
    // 用户稍后原样再点一次即可。理由：繁忙是设备**主动拒绝、指令根本没执行**（规格书 v1.5 §6.6.1，
    // 刷墨水屏/写 Flash 的忙窗口约几秒），与 Flash 写失败(0x04)/传输中断(0x09)/断连超时那种
    // 「可能真的没删掉、也可能删了一半」不同——那几类仍照旧继续删记录（幽灵图的代价换掉不可重试）。
    let deviceError = ''
    if (slotsOnDevice.length) {
      try {
        ...
        // 「设备繁忙(0x0B)」：指令被设备主动拒绝、根本没执行 → 整批中止，什么都不删（见上方口径）。
        // 必须排在下面两个分支前面：它是唯一「中止且不动任何数据」的出口。
        if (protocol.isBusyError(error)) {
          wx.hideLoading()
          console.warn(
            '[相册删除] 0x12 回了设备繁忙(0x0B)，整批中止，不删除任何数据：',
            (error && error.message) || error
          )
          // 不调记录删除接口、不 loadPhotos()：数据一点没动，选中态保留供用户直接重试。
          toast.warn({ title: protocol.BUSY_MESSAGE, icon: 'none' })
          return
        }
        ...
        if (protocol.isSkippableDeleteError(error)) {
          ...
          // Flash 写入失败(0x04)、传输中断(0x09)、断连/应答超时：设备侧「可能真的没删掉、也可能
          // 删了一半」。只记下来，不中止——记录侧照删（见上方口径）。
          // 设备忙(0x0B) 不走这里，2026-08-24 起在上面单独中止。
