...
//     导出时整幅转 EXPORT_ROTATE_DEG 画进竖向画布。
...
// 也相同，所以**后端不会报错**——但设备按 680px 一行解析，拿到 960px 一行的数据会整幅错位/斜切，
...
Page({
  ...
  // 只用于「用户看到的框」——取景框比例、烘焙后 photo-wrap 展示尺寸。
  ...
  // 共用离屏画布导出：把 src 载入后交给 draw(ctx, img) 绘制，导出为设备分辨率的 jpg 临时文件。
  ...
  // canvas 尺寸=设备物理分辨率（竖向）：后端按上传图像素量化六色帧（帧字节数=宽×高÷2），
  // 上传图必须正好是设备尺寸，这是硬约束。
  ...
  bakeEditedImage(image, g) {
    ...
    return this._exportCanvas(outW, outH, g.src, (ctx, img) => {
      ...
      // 结果页按 cropW>0 走转换链路。展示按取景方向铺；横向成图是转过 270° 的竖向文件，
      // 要再给图片元素一个反向旋转把它摆正（见 computeRotatedImgStyle）。
  ...
  //      设备分辨率新图（即「点开始投屏自动保存当前效果」，页面没有保存按钮）；
