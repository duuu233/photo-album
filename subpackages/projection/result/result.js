  ...
  async runRealProjection() {
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        // ⚠️ 用户侧只给「图片文件错误，请尝试通过工具转码后重新投屏」这一句（2026-09-03 二改）。
        // 文案沿革：
        //   ① 「后端返回的不是…六色4bpp帧：收到 X 字节…」——帧自 2026-07-22 起就不是我方后端出的
        //      （seekink 抖动接口生成），这句把用户和客服全引到我方接口上去；
        //   ② 「图片转化出错，请重新投屏」——不再甩锅后端，但用户照着做只会原样再失败一次：
        //      同一张图重投多少次都是同一个结果（帧字节数只跟上传图像素走，重投不改变它）；
        //   ③ 现文案——已定位到根因是**这张图片文件本身**在端上出图那一步过不去
        //      （预览页恒导出设备物理分辨率，canvas 解码/导出失败时会静默回退原图，
        //       原图带着原比例进抖动接口，帧字节数必然对不上），所以引导用户换一张或转码后再投，
        //      这是当前唯一能自救的动作。详见 docs/changes/2026-09-03-AI图投屏帧字节数对不上定位.md。
        ...
        if (frameData.length !== expected4bpp) {
          ...
          throw new Error('图片文件错误，请尝试通过工具转码后重新投屏')
        }
